...
Number the images (1,2,3, etc.) instead of creating figures.

Provide a List of figures (full citation list) corresponding with the image numbers.
...
Go to EasyCite or RMIT Havard referencing printable guide.

Choose item type Other sources - sub-type Language other than English for in-text and reference list rule as well as examples.
...
Start a new page with the title/heading Appendix or Appendices (if many).

Number appendices in order of appearance and differently from how chapters are numbered in the report. E.g., ‘Chapter 1’, ‘Chapter 2’, etc. ‘Appendix A’, ‘Appendix B’, etc.

Place a short citation, Author (year), in the appendix. In in-text citation, mention the appendix labels. E.g., ... (see Appendix A).

Add the full citation in the reference list.
...
Use the original author’s name in the in-text citation.

Mention the translator(s)’ name in the references after the title of the work: (translator’s last name initials trans).
...
In the image caption, mention the screenshot timestamp (sec. minute:second) in the video. Note that screenshots are considered images. E.g., Figure 1: Academic Educational Materials (2016) What is a case study? [video], YouTube website, sec. 00:27

In-text citation: Refer to Figure numbers when you want to mention the images. E.g., Figure 1 (Academic Educational Materials 2016) illustrates...… (see Figure 2).
...
Choose item type Audiovisual materials - sub-type Streaming video / YouTube video for in-text and reference list rules as well as examples.
...
Choose item type Using RMIT Havard (or Other sources) - sub-type Personal communication for in-text and reference list rules as well as examples.
...
In LibrarySearch box, put in your search terms i.e. Chapter title => click Search.

Under the filter Resource Type, choose Book Chapters.
...
In LibrarySearch box, put in your search terms => select Search.

Under the filter Resource Type, select 'Books'.
...
In LibrarySearch box, put in the search term Passport => click Search.

Choose the result type Database: Passport.

Under Full text availability, click on Passport to access the database.
