...
class EmbeddingStore:
    ...
    def delete_document(self, doc_id: str) -> bool:
        ...
        """
        if self._use_chroma and self._collection is not None:
            matches = self._collection.get(where={"doc_id": doc_id}, include=["metadatas"])
            ids = matches.get("ids", [])
            if not ids:
                return False
    ...
    def _format_chroma_results(response: dict[str, Any]) -> list[dict[str, Any]]:
        """Convert a Chroma query response to the store's public result shape."""
        ...
        return [
            {
                ...
                # Chroma's inner-product distance is 1 - dot_product.
                "score": 1.0 - distance,
            }
