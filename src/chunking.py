...
class SentenceChunker:
    ...
    def chunk(self, text: str) -> list[str]:
        ...
        for part in parts:
            if part in [". ", "! ", "? ", ".\n"]:
                sentences_clean.append((current + part).strip())
                current = ""
            else:
                current += part
        if current.strip():
            sentences_clean.append(current.strip())
            
        chunks = []
        for i in range(0, len(sentences_clean), self.max_sentences_per_chunk):
            chunk = " ".join(sentences_clean[i:i+self.max_sentences_per_chunk])
            chunks.append(chunk)
        return chunks
...
class ChunkingStrategyComparator:
    ...
    def compare(self, text: str, chunk_size: int = 200) -> dict:
        ...
        def stats(chunks):
            return {
                "count": len(chunks),
                "avg_length": sum(len(c) for c in chunks) / max(1, len(chunks)),
                "chunks": chunks
            }
