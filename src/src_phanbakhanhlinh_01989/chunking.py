...
class RecursiveChunker:
    ...
    def _split(self, current_text: str, remaining_separators: list[str]) -> list[str]:
        ...
        if len(parts) == 1:
            # Separator not present in this text; fall back to the next one.
            return self._split(current_text, rest)

        # Split any oversized piece further, then greedily re-merge small
        # pieces back up to chunk_size so we don't emit one chunk per token.
        pieces: list[str] = []
        for part in parts:
            if not part:
                continue
