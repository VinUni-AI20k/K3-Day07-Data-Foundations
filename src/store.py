...
class EmbeddingStore:
    ...
    def _search_records(self, query: str, records: list[dict[str, Any]], top_k: int) -> list[dict[str, Any]]:
        ...
        def make_result(record: dict[str, Any], score: float) -> dict[str, Any]:
            return {
                "id": record["id"],
        ...
        scored = [make_result(record, _dot(query_vector, record["embedding"])) for record in records]
    ...
    def search_with_filter(self, query: str, top_k: int = 3, metadata_filter: dict = None) -> list[dict]:
        ...
        if self._use_chroma and self._collection is not None:
            ...
            raw = self._collection.query(
                query_embeddings=[query_embedding], n_results=top_k, where=metadata_filter or None
            )
            ids = raw.get("ids", [[]])[0]
            contents = raw.get("documents", [[]])[0]
            metadatas = raw.get("metadatas", [[]])[0]
