...
class EmbeddingStore:
    ...
    def _make_record(self, doc: Document) -> dict[str, Any]:
        ...
        record = {
            # Chroma requires unique record IDs. A document may be added more than
            # once during an experiment, so do not use doc.id alone.
    ...
    def add_documents(self, docs: list[Document]) -> None:
        ...
        if self._use_chroma and self._collection is not None:
            try:
                ...
                # The in-memory copy remains authoritative, so optional Chroma
                # failures never prevent the required lab functionality.
                self._use_chroma = False
